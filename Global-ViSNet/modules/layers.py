    ...
    def forward(self, batched_data, mask_2d=None):
        ...
        if not self.no_2d:
            degree_feature = self.in_degree_encoder(
                in_degree) + self.out_degree_encoder(out_degree)
...
class MoleculeAttnBias(nn.Module):
    ...
    def forward(self, batched_data, mask_2d=None):
        ...
        if not self.no_2d:
            ...
            if mask_2d is not None:
                spatial_pos_bias = spatial_pos_bias * \
        ...
        if not self.no_2d:
            ...
            if self.edge_type == 'multi_hop':
                ...
                if self.multi_hop_max_dist > 0:
                    spatial_pos_ = spatial_pos_.clamp(
                        0, self.multi_hop_max_dist)
...
class ViSRGCFeatureV1(nn.Module):
    ...
    def forward(self, batched_data):
        ...
        no_pos_mask =  ~(pos.eq(0).all(dim=-1).all(dim=-1)) # [n_graph, ]
        
        node_feature = self.atom_encoder(x).sum(dim=-2) # [n_graph, n_nodes, n_hidden]
        padding_mask = x.eq(0).all(dim=-1) # [n_graph, n_nodes]
        
        n_graph, n_node, _ = pos.shape
        # n_graphs, n_nodes, n_nodes, 3
        delta_pos = pos.unsqueeze(1) - pos.unsqueeze(2)
        dist = delta_pos.norm(dim=-1).view(-1, n_node, n_node)
        delta_pos /= dist.unsqueeze(-1) + 1e-5
        
        # [n_graph, n_nodes, 1, 1, n_hidden] * [n_graph, n_nodes, n_nodes, 3, 1]
...
class ViSRGCFeatureV2(nn.Module):
    ...
    def __init__(self, num_atoms, num_heads, num_edges, n_layers, embed_dim, num_kernel, no_share_rpe=False):
        ...
        self.atom_encoder = nn.Embedding(
            num_atoms + 1, embed_dim, padding_idx=0)
    ...
    def forward(self, batched_data):
        ...
        node_feature = self.atom_encoder(x).sum(
            dim=-2)  # [n_graph, n_nodes, n_hidden]
        padding_mask = x.eq(0).all(dim=-1)  # [n_graph, n_nodes]
...
class AtomTaskHead(nn.Module):
    ...
    def forward(
        ...
        q = (
            self.q_proj(query).view(
                bsz, n_node, self.num_heads, -1).transpose(1, 2)
            * self.scaling
        )
        k = self.k_proj(query).view(
            bsz, n_node, self.num_heads, -1).transpose(1, 2)
        v = self.v_proj(query).view(
            bsz, n_node, self.num_heads, -1).transpose(1, 2)
