...
from .layers import AtomFeature, MoleculeAttnBias, ViSRGCFeatureV1, AtomTaskHead
...
class ViSNetEncoder_V1(nn.Module):
    ...
    def forward(
        ...
        if self.molecule_3d_bias is not None and not (batched_data["pos"] == 0).all():
            attn_bias_3d, angle_node_features, delta_pos = self.molecule_3d_bias(batched_data)
            if mask_3d is not None:
                angle_node_features, delta_pos = angle_node_features * mask_3d[:, None, None], delta_pos * mask_3d[:, None, None, None]
            ...
            x[:, 1:, :] = x[:, 1:, :] + angle_node_features * 0.01
